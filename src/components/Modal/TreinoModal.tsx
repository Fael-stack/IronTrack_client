...
interface TreinoModalProps {
  ...
  onClose: () => void;
  onSave?: (treino: Omit<Treino, "_id">) => void; // se fornecido, salvará via callback (ex: treinador)
}
...
export default function TreinoModal({ editTreino, onClose, onSave }: TreinoModalProps) {
  ...
  // Salvar: se onSave foi passado (treinador), usa onSave e espera que o caller envie userId;
  // Caso contrário (aluno), faz POST/PUT diretamente para /treinos
  const handleSave = async () => {
    ...
    if (onSave) {
      // callback do treinador — quem chamou deve incluir userId ao enviar ao backend
      onSave({
        name: treino.name,
    ...
    // Fluxo padrão do aluno: salva direto no backend
    try {
