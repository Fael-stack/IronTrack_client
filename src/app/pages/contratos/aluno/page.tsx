...
export default function AlunoContratosPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Alunos recebidos:", alunosRes.data);
        setAlunos(
  ...
  // Criar contrato (com redirecionamento automático pro chat)
  const criarContrato = async (treinadorId: string) => {
    ...
    try {
      const res = await axios.post("http://localhost:4000/contracts", {
        ...
        status: "ativo",
        preco: 0,
        descricao: 'Contrato criado via painel do aluno', // mudar dps isso e o preco
      });
      ...
      // Redirecionar direto pro chat do contrato recém-criado
  ...
  // 🔹 Função para abrir chat de um contrato existente
          ...
          {alunos.map((a) => (
            <div
              ...
              className={`${styles.workoutItem} ${
                alunoSelecionado?._id === a._id ? styles.active : ""
              }`}
      ...
      <main className={styles.main}>
        {alunoSelecionado ? (
          ...
          <div className={styles.emptyState}>
            Selecione um aluno para visualizar contratos.
          </div>
        )}
      </main>
    </div>
